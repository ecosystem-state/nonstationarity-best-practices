...
gam_aksalmon_2 <- gam(response ~ 1+s(n_date, by = driver, bs = "gp",k=K_AK), data = dat_aksalmon)
...
ggplot(resids, aes(date, resid)) + 
  ...
  facet_wrap(~ Model, ncol=1) + ggtitle("GAM Residuals")
```
...
ggplot(resids, aes(date, resid)) + 
  ...
  geom_smooth() + 
  theme_classic() + xlab("Date") + ylab("Residuals") + 
  facet_wrap(~ Model, ncol=1, scale="free_y") + ggtitle("DLM Residuals")
```
