...
resid_df <- data.frame(Model = "Time-Varying Large Zooplankton", 
                       resid = resid(zoop_slope_int),
                       year = full_data$year)
resid_df2 <- data.frame(Model = "Time-Invariant Large Zooplankton", 
                       resid = resid(zoop),
                       year = full_data$year)
...
p_values<-data.frame(p_value=c(summary(gam(resid(zoop_slope_int)~s(full_data$year)))$s.pv,
           ...
           Model=c("Time-Varying Large Zooplankton","Time-Invariant Large Zooplankton"))
...
resid_df1 <- data.frame(Model = "Time-Varying Zooplankton", 
                       resid = resid(gam1),
...
p_values<-data.frame(p_value=c(summary(gam(resid(gam1)~s(full_data$year)))$s.pv,
           ...
           Model=c("Time-Varying Zooplankton","Smooth by Zooplankton","Smooth by Year"))
...
geom_line() + ylab("Time-varying slope") + xlab("Year") + theme_classic()
```
...
Model residuals for the time-varying water clarity - zooplankton model compared to a time-invariant model where large zooplankton is used as a covariate without *a priori* defined time peariods. p-value denotes the significance of the trend in residuals through time.
:::
...
#| out-width: "100%"
par(mfrow = c(1,2))
plotQQunif(simulationOutputzoop, main="Time-Invariant Large Zooplankton")
plotQQunif(simulationOutputzoop_slope_int, main="Time-Varying Large Zooplankton")
par(mfrow = c(1, 1))
```
...
plotQQunif(simulationOutput1, main="Time-Varying Zooplankton")
par(mfrow = c(1,1))
```
