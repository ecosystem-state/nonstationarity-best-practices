...
dataset_names <- data.frame(Dataset=c('mean_sec_scale','mean_totpuf_scale', 'mean_chl_scale','Large_scale'),
  Data=c("Water Clarity","Total Phosphorus","Chlorophyll","Large Zooplankton"))
 
analysis_long2<-analysis_long%>%left_join(dataset_names)

facet_labeller <- function(variable,value){
  return(dataset_names[value])
}
ts <- ggplot(data=analysis_long2%>%
         filter(Dataset=="mean_sec_scale"|Dataset=="mean_totpuf_scale"|
                 Dataset=="mean_chl_scale"| Dataset=="Large_scale"), aes(y=values, x=year4, group=Data, col=Data)) +
