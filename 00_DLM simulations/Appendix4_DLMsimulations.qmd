...
crossref:
  custom:
    - kind: float
      key: suppfig
      latex-env: suppfig
      reference-prefix: Figure S4.
      space-before-numbering: false
      latex-list-of-description: Supplementary Figure
    - kind: float
      key: supptab
      latex-env: supptab
      reference-prefix: Table S4.
      space-before-numbering: false
      latex-list-of-description: Supplementary Table
---

# Appendix 4: DLM Simulations

## Overview

The simulation code for this comparison is in [simulations.R](https://github.com/ecosystem-state/nonstationarity-best-practices/blob/main/DLM%20simulations/simulations.R). Complete code for this file can be found [here](https://github.com/ecosystem-state/nonstationarity-best-practices/blob/main/DLM%20simulations/Appendix4_DLMsimulations.qmd). As a brief overview, we are interested in understanding what factors of a DLM contribute to models with time-dependent slopes not performing well - specifically, we found with the GoA salmon dataset that fitting both a time-dependent slope and a time-dependent intercept had convergence issues. All of these models are constructed using the `MARSS` R package, and we are only considering models with time-dependent intercepts and slopes here.
...
For each simulated dataset, we used the `MARSS` package in R; for fast fitting we used the `TMB` estimation (method = "marssTMB"). We generated a number of summary statistics; because we are focused here on the ability to recover trends in the slope parameters, we focus here on the correlation between the estimated time-dependent slope trend and true time-dependent slope trend.
